...
export default async function ApplyPage({ params: { announcementId, moduleId } }) {
    const apply = await getApply(moduleId);

    const submit = await getSubmitApplyReference(apply.id);

    if (submit.aplId !== null) return <RedirectEditPage announcementId={announcementId} moduleId={moduleId} />;
