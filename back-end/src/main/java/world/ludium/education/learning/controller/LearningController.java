...
import world.ludium.education.learning.service.CurriculumService;
import world.ludium.education.learning.service.EnhancedArticleService;
import world.ludium.education.learning.service.LearningService;
import world.ludium.education.learning.service.MissionService;
...
public class LearningController {
  ...
  public ResponseEntity<Object> getAllMonitor(@PathVariable UUID learningId) {
    try {
      ...
      if (monitoringList.isEmpty()) {
        return responseUtil.getNoSuchElementExceptionMessage("학습 현황 데이터가 없습니다.", "");
      }
  ...
  public ResponseEntity<Object> getArticleSubmit(@PathVariable UUID articleId,
    ...
    try {
      return ResponseEntity.ok(
          enhancedArticleService.getArticleSubmit(articleId, ludiumUser.getId()));
    } catch (NoSuchElementException nse) {
  ...
  public ResponseEntity<Object> createArticleSubmit(@PathVariable UUID articleId,
    ...
    if (enhancedArticleService.isExistArticleSubmit(articleId, ludiumUser.getId())) {
      return responseUtil.getDuplicateExceptionMessage(
          new ResponseException("이미 제출된 아티클 입니다.", ""));
    }
  ...
  public ResponseEntity<Object> updateLearning(@RequestBody Learning learning,
    ...
    if (!ludiumUser.getId().equals(learning.getUsrId())) {
      return responseUtil.getForbiddenExceptionMessage(
          new ResponseException("학습 수정자 정보가 일치하지 않습니다.", ""));
    }
  ...
  public ResponseEntity<Object> updateCurriculum(@RequestBody Curriculum curriculum,
    ...
    if (!ludiumUser.getId().equals(curriculum.getUsrId())) {
      return responseUtil.getForbiddenExceptionMessage(
          new ResponseException("커리큘럼 수정자 정보가 일치하지 않습니다.", ""));
    }
  ...
  public ResponseEntity<Object> updateMissionSubmit(@PathVariable UUID missionId,
    ...
    if (!ludiumUser.getId().equals(enhancedMissionSubmit.getUsrId())) {
      return responseUtil.getForbiddenExceptionMessage(
          new ResponseException("미션 제출자 정보가 일치하지 않습니다.", ""));
    }
