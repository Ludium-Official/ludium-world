...
public class ProfileController {
  ...
  public ResponseEntity<Object> getAllApplication(@PathVariable UUID userId) {
    try {
      ...
      if (applicationList.isEmpty()) {
        return responseUtil.getNoSuchElementExceptionMessage("지원서 데이터가 없습니다.", "");
      }
  ...
  public ResponseEntity<Object> getAllDetailedAnnouncement(@PathVariable UUID usrId) {
    try {
      var detailedAnnouncementList = detailedAnnouncementService.getAllDetailedAnnouncementByWorker(
          usrId);

      if (detailedAnnouncementList.isEmpty()) {
        return responseUtil.getNoSuchElementExceptionMessage("작업 데이터가 없습니다.", "");
      }
  ...
  public ResponseEntity<Object> getTop4DetailedAnnouncement(@PathVariable UUID usrId) {
    try {
      var detailedAnnouncementList = detailedAnnouncementService
          .getTop4DetailedAnnouncementByWorker(usrId);
  ...
  public ResponseEntity<Object> getAllMission(@PathVariable UUID usrId) {
    try {
      ...
      if (missionDtoList.isEmpty()) {
        return responseUtil.getNoSuchElementExceptionMessage("미션 데이터가 없습니다.", "");
      }
