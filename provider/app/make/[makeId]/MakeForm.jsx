...
export default function MakeForm({ id, title, content, shareable }) {
    ...
    const handleSavemake = async (e) => {
        ...
        const createmakeResponse = await fetchWithRetry(`/article/${id}`, {
            method: "PUT",
            body: formData,
        });
    ...
    return (
        <form
            className={makestyle["make-new-wrapper"]}
            onSubmit={handleSavemake}
        >
            <div className={makestyle["form-button-area"]}>
                ...
                {shareable ? <button className={makestyle["form-button"]} type="button">공유하기</button> : null}
                <input
                    className={makestyle["form-button"]}
                    type="submit"
                    value="저장하기"
            ...
            <div className={makestyle["make-new-header-area"]}>
                <input
                    ...
                    readOnly
                />
                <select name="category" id="category" defaultValue={Category.MAKE} readOnly>
